...
export function BattlegroundPrep() {
  ...
  return (
    <section className="bg-prep">
      ...
      <div className="card-actions">
        <button type="button" onClick={() => store.run(store.sketchBattleground())}>
          Sketch map card
        </button>
      ...
      <BoardScaleControls onPickCard={() => setPickingCard(true)} />
        ...
        {tokens.map((token) => {
          if (token.entityId === null) {
            return (
              <li key={token.id}>
                <span
                  className={token.shape === "square" ? "token-art-mini is-square" : "token-art-mini is-circle"}
                  style={{ background: token.color ?? "#888" }}
                />
                <span>{token.shape}</span>
                <button type="button" onClick={() => store.run(store.removeToken(token.id))}>
                  Remove
                </button>
              </li>
            );
